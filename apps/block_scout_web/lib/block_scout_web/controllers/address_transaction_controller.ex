defmodule BlockScoutWeb.AddressTransactionController do
  ...
  defp items_csv(
    ...
    with {:ok, address_hash} <- Chain.string_to_address_hash(address_hash_string),
         {:ok, address} <- Chain.hash_to_address(address_hash),
         {:recaptcha, true} <-
           {:recaptcha, captcha_helper().recaptcha_passed?(recaptcha_response)} do
      address
  ...
  defp items_csv_block(
    ...
    with {:ok, address_hash} <- Chain.string_to_address_hash(address_hash_string),
         ...
         {:recaptcha, true} <-
           {:recaptcha, captcha_helper().recaptcha_passed?(recaptcha_response)} do
      address
      |> csv_export_module.export_block(from_block, to_block)
      |> Enum.reduce_while(put_resp_params(conn, file_name), fn chunk, conn ->
        case Conn.chunk(conn, chunk) do
          {:ok, conn} ->
            {:cont, conn}
  ...
  defp items_csv_block(conn, _, _, _), do: not_found(conn)
